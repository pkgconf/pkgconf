...
static bool
generate_spdx(pkgconf_client_t *client, pkgconf_pkg_t *world)
{
	int eflag;
	pkgconf_buffer_t buffer = PKGCONF_BUFFER_INITIALIZER;

	spdxtool_core_agent_t *agent = spdxtool_core_agent_new(client, strdup("_:creationinfo_1"), strdup("Default"));
	spdxtool_core_creation_info_t *creation = spdxtool_core_creation_info_new(&pkg_client, strdup(agent->spdx_id), strdup("_:creationinfo_1"));
	spdxtool_core_spdx_document_t *document = spdxtool_core_spdx_document_new(&pkg_client, spdxtool_util_get_spdx_id_int(&pkg_client, "spdxDocument"), strdup("_:creationinfo_1"));
...
usage(void)
{
	printf("usage: spdxtool [--flags] [modules]\n");
...
main(int argc, char *argv[])
{
	...
	if (!generate_spdx(&pkg_client, &world))
	{
		ret = EXIT_FAILURE;
		goto out;
	}
